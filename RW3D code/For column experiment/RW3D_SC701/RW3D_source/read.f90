...
!********************************************************************************************************
     subroutine read_parameters  ( geo, advection, dispersion, mass_trans, reaction, plume, well, plane,  &
	                               histo, source, recirculation, nsim )

     use gslib, only: generate_unit,ifcharacter,generate_name
	 ...
	 integer                           :: cell(3),icol,jrow,klay
     real*8                            :: xc(3),qxval,qyval,qzval,qmodval,vxav, vyav
...
!     Read Dispersion Parameters:
      ...
      ! Initialize the diffusion coefficients for mineral phase     - Dong
      if (nspe_min > 0) then
          do jj = nspe_aq+1, nspecie
              dispersion%MultD(jj) = 0
          end do
      end if
...
!.......... create plumes and initialize some objects:
           ...
           !if (.not.reaction%action) nzone = 1 

           call initialize_plume_ (plume,0.d0,nspecie,nzone) 
...
!..........initialize output files:
		...
		call initialize_output_files(nsim)
...
!....... write to debugging file:
	    ...
	    call list_array_ (geo%dx,fdbg,' cell size x-direction')
		call list_array_ (geo%dy,fdbg,' cell size y-direction')
		call list_array_ (geo%dz,fdbg,' cell size z-direction')
	  ...
	  if (advection%action.and. idebug >=2) then
	       ...
	       write(iunitVEL,'(a43)') 'variable="x" "y" "z" "qx" "qy" "qz" "qmod" ' 
		   write(iunitVEL,*)   'ZONE I=',nx,' J=',ny,' K=',nz,' F="POINT" '
		   do klay=1,nz
		     do jrow=1,ny
		       do icol=1,nx
			   cell(1)=icol
...
!************************************************************************************************************************************************************************************
  subroutine initialize_output_files(nsim)
      use gslib, only: generate_unit,open_fname_normal
	  use code_options
	  use global_variables, only: files_nam,fname_exit
	  implicit none
	  integer :: iunit
	  logical :: existeix, exist1
      character(len=3)                :: nsfn ! string to update the file name of part_path
      integer                           :: nsim
...
!...........Output Files
      ...
      if (iwcshot /= 0) then
	       ...
	       write(iunit,*) 'VARIABLES="ID" "Xp" "Yp" "Zp" "Mp" "Rp" "ZONE" "SPECIE" '
           !close(iunit)
	  end if

      if (iwpath == 1 ) then
           inquire(file="rw3d_part_path.dat", exist=exist1)
           if (exist1) then
               write (nsfn,'(I3.3)') nsim
               files_nam(5)='rw3d_part_path_'//trim(nsfn)//'.dat'        ! generate new particle paths     - Dong
           end if
           iunit = generate_unit(100)
