...
!************************************************************************
      subroutine print_plume_tecplot_ (plume,fname)   !printer
          ...
          use global_variables, only: StartTimeInjection,tshot,AlwaysPrintShots 
		  use list_class
		  ...
		  if (AlwaysPrintShots .or. FirstInjection) then  !if negative or start time injection always priting to file
          ...
          species1: do ispe  = 1, plume%nspecie
            zones1: do izone = 0, plume%nzone-1       
               ...
               particles1:  do        ! move all particles one time step              
                 if (.not.associated(part)) cycle zones1        
	   	         write(iunit,3) part%id,part%xp,part%yp,part%zp,part%mp,part%rp,izone,ispe 			     
			     part => part%next
			  end do particles1
		    end do zones1
		  end do species1   	  
          ...
          if (plume%time > tshot(itnext)) then
                  call locate(tshot,nt,1,nt,plume%time,itnext)
                  if (itnext<nt) itnext = itnext+1
          end if
		  ...
		  if (plume%time >= tshot(itnext)) then
	       ...
	       2 format(a8,g15.6,a2) 		  
              ...
              zones: do izone = 0, plume%nzone-1
                 ...
                 particles:  do        ! move all particles one time step              
                      if (.not.associated(part)) cycle zones        
	   	              write(iunit,3) part%id,part%xp,part%yp,part%zp,part%mp,part%rp,izone,ispe 
	               3  format(i10,x,5(x,e20.11),2(i4,x))			     
			          part => part%next
			     end do particles
		     end do zones
		    end do species   
	        ...
	        if (itnext > nt ) EndPrintShot = .TRUE.
...
!************************************************************************
    subroutine print_number_of_particles_ (plume,fname)
      ...
      integer                                 :: izone,ispe,npl,i
       ...
       if(.not.present(fname)) then
           iunit=6
           write(iunit,'(a8,x,g15.6,x,2(3x,a7,x,i9))') 'TIME =',plume%time,'MOVE =',nmove,'NPTOT =',plume%np
       else
	       ...
	       if (FirstTime) then
	          ...
	          write(iunit,*)
	          write(iunit,'(<40+55+11*npl>("-"))') 
	          write(iunit,'(95x,a)') ' (SPECIES,ZONES): ' 
	          write(iunit,*)                          
	          write(iunit,'(x,a15,x,7(x,a10),<npl>(a11))') '     TIME      ','MOVE','NPTOT','NPINJ','NPWELL','NPPLANE','NPOUT','NPSTUCK',(trim(adjustl(name(i))),i=1,npl)
	          write(iunit,'(<40+55+11*npl>("-"))')                          
	       end if 
	       i=0
	       do ispe=1,nspecie
	       ...
	       write(iunit,1) plume%time,nmove,plume%np,npInj,npWell,npPlane,npOut,npStuck,(np11(i),i=1,npl) 
	   1   format (x,g15.6,x,7(x,i10),<npl>(x,i10) )
