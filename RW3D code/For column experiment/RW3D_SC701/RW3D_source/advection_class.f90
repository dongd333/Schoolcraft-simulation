  ...
  type advection_cl
	  ...
	  integer           :: it = 1              !timeshot associated with qx,qy,qz
  ...
  contains
   ...
   subroutine print_velocity_timeshots_ (fname,advection)
       ...
       integer                        :: iunit,it
          ...
          write(iunit,*)
          write(iunit,*) ' *******************'
          write(iunit,*) ' VELOCITY TIME SHOTS'
          write(iunit,*) ' *******************'
          do it =1,advection%nt
             write(iunit,*) it,advection%time(it)
          end do
          write(iunit,*)
          close(iunit)
   end subroutine
    ...
    subroutine update_advection_timeshot_ (this,time)
       ...
       integer                           :: kper,kstp
         ...
         !loop: do iper=kper,velotime%nper
                  !do istp=kstp+1,velotime%period(iper)%NSTP
                     !itf = itf + 1
                     !if (time >= this%time(itf-1) .and. time < this%time(itf)) then
         !               exit loop
         !            end if
         !         end do
         !      end do loop
         ...
         if (itf /= it + 1) then
            write(*,*) 'WARNING: NOT CONSECUTIVE VELOCITIES'
         end if 
