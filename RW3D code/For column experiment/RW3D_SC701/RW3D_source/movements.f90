...
!
!*************************************************************************************************   
   function calculate_plume_time_step (tc,plume,geo,advection,dispersion,reaction,mass_trans) result(dt)
      use global_variables, only: UNEST,DtStep,courant,peclet,calcul_time_method,DaKINETIC,DaDECAY,DaMMT,StartTimeInjection,npMobile
	  ...
	  real*8                       :: dtvelo
          ...
          !...if no particle in plume, go to the next injection time
          !if(plume%np <= 0) then
          !    dt = StartTimeInjection - plume%time
          !    return
          !endif
          ...
          !...check that time step is not larger than a time step in modflow
          dtvelo = advection%time(advection%it)-advection%time(advection%it-1)
          
          dt = min(dt,0.5*dtvelo)
          
          !...modify time step when changing the velocity field
          if (plume%time < advection%time(advection%nt) .and. plume%time + dt > advection%time(advection%it)) then
               dt = advection%time(advection%it)-plume%time 
          end if
          
          !...modify time step to match start of injection
          if (plume%time< StartTimeInjection .and. plume%time + dt > StartTimeInjection) then 
              dt = StartTimeInjection-plume%time
          end if          
