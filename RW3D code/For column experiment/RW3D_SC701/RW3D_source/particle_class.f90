   ...
   !************************************
    subroutine print_position_particle_ (this,fname, nmove)
	   ...
	   real*8                                 :: qqp
		   ...
		   if (step == 0) then
               if (this%position%tp > 47.9 .AND. this%position%tp < 48.1 .OR. this%position%tp > 623.9) then    ! Just for biodegration-column
...
!***************************************************************************************************
    subroutine update_properties_particle_1 (this,geo,advection,dispersion,reaction)
       ...
       if (dispersion%action) then
           ! Because all species are defined with a multiplier, including immobile phase (0), then the update is the same
           !if(.not.SpeciesDispersionDependent) then
	        ...
	        !else
	           !if (this%specie%num <= nspe_aq) then
                 ...
                 !write(*,*) 'diffusion: ', this % prop % dm, 'species: ', this%specie%num      ! Test diffusion and species        - Dong
	            !end if
	        !end if
       end if !; end if
...
!  ***********************************************************************************************************************
    subroutine update_velocity_particle_ (this,geo,advection,dispersion)
	   use geometry_class
	   use advection_class
	   use dispersion_class
	   use heterogeneity_flags
         ...
         !if (vel_homogeneous== .FALSE. .or. nmove == 1) then
		    if (advection%action) then
                if (this%control%switchcell) call vel_faces_ (advection,this%cell%num,this%cell%qxface,this%cell%qyface,this%cell%qzface)        
			        this%vel%qpL = vel_point_linear (this); end if !; end if        
         !if (disp_homogeneous== .FALSE. .or. nmove == 1) then
    ...
    !**********************************************************
	    ...
	    do ispe=1,nspecie
	       if (sorptionACTION .and. sorptionTYPE == 'LINEAR') then
               ...
               Rim(ispe,1) = particle % sorption % linear_sorp % Rim(ispe,1)
	       else 
               R(ispe)     = 1.d0
               Rim(ispe,1) = 1.d0
	       end if
