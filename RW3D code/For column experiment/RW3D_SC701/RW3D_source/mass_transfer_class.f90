...
!********************************************************************************************************************
  ...
  function read_multirate_ (mass_trans,unit,geo) result (this) 
      ...
      do i=1,nzoneim
         ...
         if (mass_trans)  this%poro_imm(i)  = read_array_ (file,ivar,const,flag,geo%nx,geo%ny,geo%nz)

         read(unit,*) file,const,ivar,flag
         if (mass_trans)  this%alphap(i)    = read_array_ (file,ivar,const,flag,geo%nx,geo%ny,geo%nz)
      end do
  ...
  subroutine delete_multirate_ (this)
     ...
     type(multirate_cl), intent(inout)          :: this
  ...
  function inquire_homogeneity_multirate_ (this) result (flag)
      ...
      type(multirate_cl), intent(in)        :: this
      logical                               :: flag
      integer                               :: i
...
!********************************************************************************************************************
 ...
 use array_class
  ...
  private
  public :: spherical_diffusion_cl                        !classes
  public ::                                       &       !methods
  ...
  function read_spherical_diffusion_ (reaction,unit,geo) result (this) 
	  ...
	  real*8                        :: const
	  integer                       :: ivar,flag
	  type(spherical_diffusion_cl)  :: this
  ...
  subroutine delete_spherical_diffusion_ (this)   !destructor
	 ...
	 implicit none
     integer                                     :: nzone
	 type(spherical_diffusion_cl), intent(inout) :: this
		if (associated(this%nzones))   deallocate (this%nzones)
		if (associated(this%poro_imm)) deallocate (this%poro_imm)
		if (associated(this%alphap))   deallocate (this%alphap)
  end subroutine
  ...
  function inquire_homogeneity_spherical_ (this) result (flag)
      use array_class
	  implicit none
	  type(spherical_diffusion_cl), intent(in) :: this
	  logical                                  :: flag
	        flag = .TRUE.
...
!********************************************************************************************************************
 ...
 use array_class
  ...
  private
  public :: layered_diffusion_cl                        !classes
  public ::                                     &       !methods
  ...
  function read_layered_diffusion_ (reaction,unit,geo) result (this) 
	  ...
	  real*8                        :: const
	  integer                       :: ivar,flag
	  type(layered_diffusion_cl)    :: this
  ...
  subroutine delete_layered_diffusion_ (this)   !destructor
	 ...
	 type(layered_diffusion_cl), intent(inout)   :: this
  ...
  function inquire_homogeneity_layered_ (this) result (flag)
      use array_class
	  implicit none
	  type(layered_diffusion_cl), intent(in) :: this
	  logical                                :: flag
...
!********************************************************************************************************************
 ...
 implicit none
  ...
  private
  public :: cylindrical_diffusion_cl                        !classes
  public ::                                         &       !methods
  ...
  function read_cylindrical_diffusion_ (reaction,unit,geo) result (this) 
	  ...
	  real*8                            :: const
	  integer                           :: ivar,flag
	  type(cylindrical_diffusion_cl)    :: this
  ...
  subroutine delete_cylindrical_diffusion_ (this)   !destructor
	 ...
	 implicit none
     integer                                       :: nzone
	 type(cylindrical_diffusion_cl), intent(inout) :: this
		if (associated(this%nzones))   deallocate (this%nzones)
		if (associated(this%poro_imm)) deallocate (this%poro_imm)
		if (associated(this%alphap))   deallocate (this%alphap)
  end subroutine
  ...
  function inquire_homogeneity_cylindrical_ (this) result (flag)
      use array_class
	  implicit none
	  type(cylindrical_diffusion_cl), intent(in) :: this
	  logical                                  :: flag
	  integer                                  :: i
	        flag = .TRUE.
            if(associated(this % poro_imm) .and. length_array_ (this % poro_imm) > 1)  flag = .FALSE.
...
!********************************************************************************************************************
 ...
 use array_class
  ...
  private
  public :: power_distribution_cl                        !classes
  public ::                                      &       !methods
  ...
  function read_power_distribution_ (reaction,unit,geo) result (this)
      ...
      real*8                        :: const
      integer                       :: ivar,flag
      type(power_distribution_cl)   :: this
         read(unit,*) nzoneim
              if (reaction) then
                   allocate (this%nzones)
                   this%nzones=nzoneim; endif
  ...
  subroutine delete_power_distribution_ (this)   !destructor
     ...
     implicit none
     integer                                     :: nzone
     type(power_distribution_cl), intent(inout)  :: this
        if (associated(this%nzones))     deallocate (this%nzones)
  ...
  function inquire_homogeneity_power_ (this) result (flag)
      use array_class
      implicit none
      type(power_distribution_cl), intent(in)  :: this
      logical                                  :: flag
      integer                                  :: i
            flag = .TRUE.
...
!********************************************************************************************************************
 ...
 use array_class
  ...
  private
  public :: lognormal_distribution_cl                        !classes
  public ::                                          &       !methods
  ...
  type lognormal_distribution_cl
	  integer,        pointer   :: nzones   => null()   !number of immobile zones
	  type(array_cl), pointer   :: btot     => null()   !total capacity coefficient
  ...
  function read_lognormal_distribution_ (reaction,unit,geo) result (this) 
	  ...
	  type(lognormal_distribution_cl)   :: this
         read(unit,*) nzoneim
		      if (reaction) then
			       allocate (this%nzones)
				   this%nzones=nzoneim; endif
  ...
  subroutine delete_lognormal_distribution_ (this)   !destructor
	 ...
	 implicit none
     integer                                     :: nzone
	 type(lognormal_distribution_cl), intent(inout)  :: this
		if (associated(this%nzones))     deallocate (this%nzones)
		if (associated(this%btot))       deallocate (this%btot)
  ...
  function inquire_homogeneity_lognormal_ (this) result (flag)
      use array_class
	  implicit none
	  type(lognormal_distribution_cl), intent(in)  :: this
	  logical                                  :: flag
	  integer                                  :: i
	        flag = .TRUE.
            if(associated(this % btot)       .and. length_array_ (this % btot)     > 1)  flag = .FALSE.
...
!********************************************************************************************************************
 module composite_media_class   ! class defining mass transfer processes
  ...
  private
  public :: composite_media_cl                            !classes
  public ::                                         &     !methods
  ...
  type composite_media_cl
	  ...
	  type(multirate_cl),             pointer :: Multirate(:)            => null()
	  type(spherical_diffusion_cl),   pointer :: SphericalDiffusion(:)   => null()
	  type(layered_diffusion_cl),     pointer :: LayeredDiffusion(:)     => null()
	  type(cylindrical_diffusion_cl), pointer :: CylindricalDiffusion(:) => null()
  end type
  ...
  use cylindrical_diffusion_class
	 implicit none
     integer                                 :: i
	 type(composite_media_cl), intent(inout) :: this
  ...
  function inquire_homogeneity_composite_ (this) result (flag)
      ...
      use cylindrical_diffusion_class
	  implicit none
	  type(composite_media_cl), intent(in)     :: this
	  logical                                  :: flag
	  integer                                  :: i
	        flag = .TRUE.
            if (associated(this%Multirate))  then
...
!*********************************************************************************************************************
  ...
  public ::                                     &       !methods
       ...
       type mass_trans_cl
           ...
           type(multirate_cl),               pointer   :: multirate            => null()
           type(spherical_diffusion_cl),     pointer   :: sphericalDiff        => null()
           type(layered_diffusion_cl),       pointer   :: layeredDiff          => null()
           type(cylindrical_diffusion_cl),   pointer   :: cylindricalDiff      => null()
           type(power_distribution_cl),      pointer   :: power                => null()
           type(lognormal_distribution_cl),  pointer   :: logNormal            => null()
           type(composite_media_cl),         pointer   :: composite            => null()
       end type
