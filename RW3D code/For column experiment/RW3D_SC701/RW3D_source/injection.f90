 ...
 !subroutine inject_particles ( plume, geo, advection, reaction, source )
 subroutine inject_particles ( plume, geo, advection, mass_trans, reaction, source )
	   use reaction_class
...
!...Initialize
       ...
       if(.not.associated(tinjold)) then
              allocate (tinjold(n))
              do isource=1,n
                  tinjold(isource) = source%num(isource)%TimeStartInj
              end do
       end if
...
!...loop over injections
 ...
 source_loop: do isource=1,n	  
          ...
          if (trim(adjustl(source%num(isource)%TypeInj)) == 'DIRAC') then
                 tinj = source%num(isource)%TimeStartInj
                 if (time >= tinj .and. tinjold(isource)>=0.d0) then
                    !call inject_particles_one_source ( plume, geo, advection, reaction, source, isource )
                    call inject_particles_one_source ( plume, geo, advection, mass_trans, reaction, source, isource )
                    tinjold(isource) = -1.d0
                 end if                       
                 cycle source_loop 
          end if              
          ...
          if (trim(adjustl(source%num(isource)%TypeInj)) == 'GENERAL') then              
                 nt    = source%num(isource)%timefunct%nt
                 ti    = source%num(isource)%timefunct%time(1)
                 tf    = source%num(isource)%timefunct%time(nt)
                 freq  = source%num(isource)%freq           
                 if (.not.associated(it)) then
                        allocate (it(n))
                        it = 1
                 end if
                 if (it(isource) >= nt) cycle source_loop                
                 if(.not.associated(counter)) then
                        allocate (counter(n))
                        counter = 1
                 end if               
                 if (counter(isource) < freq) then
                      counter(isource) = counter(isource) + 1
                      cycle source_loop
                 end if
                 t1 = source%num(isource)%timefunct%time(it(isource))
                 t2 = source%num(isource)%timefunct%time(it(isource)+1)                
                 if (time <= t2) then
                      Jm = evaluate_timefunction_upwind_ (source%num(isource)%timefunct,time)
                      Mass = Jm *(time-tinjold(isource)) !total mass injected                      
                      if (Mass > 0.d0) then
                         np = int(Mass/source%num(isource)%pmass+0.5d0)            !calculate the number of particles to inject
                         if (np>MinNumPart) then
                           source%num(isource)%np = np                        !inject at least one particle         
                           Mass = source%num(isource)%np*source%num(isource)%pmass   !correct mass to match mass flux
                           source%num(isource)%pmass = Mass/dfloat(source%num(isource)%np)                        
                           !call inject_particles_one_source ( plume, geo, advection, reaction, source, isource )
                           call inject_particles_one_source ( plume, geo, advection, mass_trans, reaction, source, isource )
                           tinjold(isource) = time                       
                           counter(isource) = 1
                         end if
                      end if
                      
                  else if (time > t2) then
                      Jmt   = evaluate_timefunction_upwind_ (source%num(isource)%timefunct,time)
                      Jmold = evaluate_timefunction_upwind_ (source%num(isource)%timefunct,tinjold(isource)) 
                      Mass = Jmold *(t2-tinjold(isource))+Jmt*(time-t2)  !total mass injected                       
                      if (Mass > 0.d0) then
                         np = int(Mass/source%num(isource)%pmass+0.5d0)
                         if (np>MinNumPart) then
                            source%num(isource)%np = np                         
                            Mass = source%num(isource)%np*source%num(isource)%pmass
                            source%num(isource)%pmass = Mass/dfloat(source%num(isource)%np)                        
                            !call inject_particles_one_source ( plume, geo, advection, reaction, source, isource )
                            call inject_particles_one_source ( plume, geo, advection, mass_trans, reaction, source, isource )
                            tinjold(isource) = time                       
                            counter(isource) = 1
                            it(isource) = it(isource)+1
                         end if
                      end if                         
                 end if
                 cycle source_loop                
...
!       ========================================
        ...
        if (xinj_1 == xinj_2 .and.  zinj_1 == zinj_2) then
            ...
            ! find the end-point cells defining the line
             ...
             ! get number of particles to be used       

             npsum = 0

             do j=j2,j3
  
                  npsum = npsum + dint(np*qm(j)/sumq+0.5d0)
             ...
             if (np /= npsum) then
                    ...
                    write(unit,*) 
			        write(unit,*) 'WARNING: Number of Injected Particles Changed to adapt to fluxes '
			        write(unit,*)
                    write(unit,*) '         Injection......: ',kinj
			        write(unit,*) '         Particles......: ',npsum
			        write(unit,*)

                    write(*,*) 
			        write(*,*) 'Number of Injected Particles Changed to adapt to fluxes '
			        write(*,*)
                    write(*,*) '         Injection......: ',kinj
			        write(*,*) '         Particles......: ',npsum
			        write(*,*)                
             ...
             do j=j2,j3
                 ...
                 npblock = dint(np*qm(j)/sumq+0.5d0)
	             if (ny /= 1) dy = value_array_ (geo%dy,1,j,1)         
             ...
             if (npsum /= jpsum) then
                  stop ' Number of particles during injection is inconsitent'
             end if
        ...
        elseif (xinj_1 == xinj_2 .and. yinj_1 == yinj_2) then
            ...
            ! find the end point cells defining the line
             ...
             do k=k2,k3
                  ...
                  !if (nz /= 1) dz = value_array_ (geo%dz,1,j,1)
             ...
             do k=k2,k3
                 ...
                 npsum = npsum + dint(np*qm(k)/sumq+0.5d0)
             ...
             if (np /= npsum) then
                    ...
                    write(unit,*) 
			        write(unit,*) 'WARNING: Number Injected Particles Changed to adapt to fluxes'
			        write(unit,*)
                    write(unit,*) '         Injection......: ',kinj
			        write(unit,*) '         Particles......: ',npsum
			        write(unit,*)
...
!       ==================================================
      ...
      ! get velocities
	    ...
	    iz = source % num (kinj) % par % iz
             ...
             do k=1,size(ix)
                 npsum = npsum + dint(np*qm(k)/sumq+0.5d0)
                 !!npsum = npsum + dint(np*exp(qm(k))/sumq+0.5d0)
             end do
