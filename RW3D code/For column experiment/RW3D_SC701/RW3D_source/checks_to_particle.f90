			   ...
			   if (.not.associated(well%num(iwell)%zwtop)) then
				  ...
				  ! For fully penetrating wells
	                ...
	                if ( well%num(iwell)%out == 1 ) then
                        particle%control%remove = .TRUE.
                        npOut = npOut + 1                       ! Particle out was not recorded, so added.     - Dong
                    end if
                  ...
                  if (zp >= well%num(iwell)%zwbot .and.  zp <= well%num(iwell)%zwtop) then        
			        if (well%num(iwell)%SaveBTC) call add_particle_to_btc_ (well%num(iwell)%btc(spe),id,time,mp)
	                if ( well%num(iwell)%out == 1 ) then
                        particle%control%remove = .TRUE.    ! Particles out from well did not recorded in the file, added. - Dong
                        npOut = npOut + 1
                        !write(*,*) xp1,yp1,zp,tp1,well%num(iwell)%name, well%num(iwell)%out
                        call open_fname (fname_exit,iunit)
                        write(iunit,101) xp1,yp1,zp,spe,id,tp1,well%num(iwell)%name
                        101                format(3(g15.6,x),i4,x,i8,x,g15.6,x,A20, i4)
                    end if
	                particle%control%passwell = iwell 
	                npWell = npWell + 1
			      end if
