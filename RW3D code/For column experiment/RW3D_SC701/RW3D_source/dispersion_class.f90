  ...
  subroutine alloc_dispersion_ (this)
     use global_variables, only: nspe_aq, nspecie
     implicit none
	 type(dispersion_cl), intent(inout) :: this
		 ...
		 if (.not. associated (this % MultA))  allocate (this % MultA(nspecie))         ! The array is re-seized for all species from nspe_aq        - Dong
		 if (.not. associated (this % MultD))  allocate (this % MultD(nspecie))
  end subroutine
