...
!*************************************************************************************************   
   function calculate_plume_time_step (tc,plume,geo,advection,dispersion,reaction,mass_trans,StressTime) result(dt)
      use global_variables, only: UNEST,DtStep,courant,peclet,calcul_time_method,DaKINETIC,DaDECAY,DaMMT,StartTimeInjection,tsim
      ...
      integer                      :: loc
          ...
          if (plume%time < StressTime%time(StressTime%nt) .and. plume%time + dt > StressTime%time(StressTime%loc+1)) then
               dt = StressTime%time(StressTime%loc+1)-plume%time 
          end if
