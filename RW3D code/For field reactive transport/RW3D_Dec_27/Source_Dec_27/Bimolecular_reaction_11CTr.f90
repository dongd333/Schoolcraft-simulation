...
!************************************************************************************************************************************************************
    subroutine bimolecular_plume_reaction_one_zone (plume,izone,geo,advection,dispersion,reaction,dt,seedbimo)
        ...
        use gslib,              only: rand3, inv, sortem
        ...
        real*8                                      :: dm,dmTH,dmTV
        integer                                     :: minpartkdt = 10          !minimum number of particle allowed to create kd-tree
        logical                                     :: useKDT = .FALSE.
        real*8, allocatable                         :: distAB(:),num(:)
        real*8                                      :: Zct, KDct
        integer                                     :: Type_A                   ! Check type of A+B->C+D reaction
        
        integer                                     :: seedbimo
        ! Add for depth-dependent distribution coefficient
        !--------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        useKDT = .TRUE.     ! Use kdtree
        
        ! Reaction No. 1    - Adsorption of CT, the rate is calculated based on the ratio from Kd and desorption rate
              !initialize
              
              spA = reaction%kinetic%rxn(1)%reactants(1)                                             !species number 
              ...
              do i=1,npA           
                 ...
                 Zct = particleA%position%xp(3)     ! - Get the Z value of particle A (reactant)
                 ! Declare the ratio
                 if (Zct > 12.5) then
                     KDct = 0.2332
                 elseif (Zct <= 6.5) then
                 ...
                 kf   = KDct*0.36                   !get reaction coefficient with desorption rate as 0.36 (Phanikumar et al. 2005)           
                 ...
                 case (.TRUE.)    !reaction occurrs, so kill particle and go to next one
                           spC = reaction%kinetic%rxn(1)%products(1)
        ...
        do irxn = 2,reaction%kinetic%nreact
          ...
          elseif (reaction%kinetic%rxn(irxn)%nreactants == 2) then
             ...
             if (useKDT)    Btree => kdtree2_create(Bloc,sort=.false.,rearrange=.false.)  
      ...
      loopA: do i=1,npA !loop particles A
                 ...
                 if (useKDT) then   !use KD-Tree search algorithm
                     call kdtree2_r_nearest(tp=Btree,qv=Alocvec,r2=maxsearch*maxsearch,nfound=nf,nalloc=npB,results=nearest)
                 else               !compute distances between all B particle and the given A particle
                     if (.not.allocated(distAB)) allocate(distAB(npB)) 
                     if (.not.allocated(num))    allocate(num(npB))
                     distAB = 0.d0
                     num    = 0.d0
                     nf = 0
                     do ipB=1,npB
                         num(ipB)=float(ipB)
                         do idim=1,ndim
                            distAB(ipB) = distAB(ipB) + (particleA%position%xp(idim)-Bloc(idim,ipB))**2
                         end do
                         distAB(ipB) = sqrt(distAB(ipB))
                         if (distAB(ipB) <= maxsearch) then
                            nf = nf+1
                         end if
                     end do
                     call sortem(1,npB,distAB,1,num,aa,aa,aa,aa,aa,aa)
                     do ipB=1,nf
                        nearest(ipB)%DIS = distAB(ipB)
                        nearest(ipB)%IDX = ipB
                     end do
                 end if
          ...
          loopB: do ii=1,nf 
                    ...
                    do idim=1,ndim
                        if (ActiveDim(1).and.testDIM(1)) then
                            if (useKDT)         particleB % position % xp(1) = Btree%the_data(idim,ipB)
                            if (.not.useKDT)    particleB % position % xp(1) = Bloc(idim,ipB)
                            testDIM(1) = .FALSE.
                        else if (ActiveDim(2).and.testDIM(2)) then
                            if (useKDT)         particleB % position % xp(2) = Btree%the_data(idim,ipB)
                            if (.not.useKDT)    particleB % position % xp(2) = Bloc(idim,ipB)
                            testDIM(2) = .FALSE.
                        else if (ActiveDim(3).and.testDIM(3)) then
                            if (useKDT)         particleB % position % xp(3) = Btree%the_data(idim,ipB)
                            if (.not.useKDT)    particleB % position % xp(3) = Bloc(idim,ipB)
                            testDIM(3) = .FALSE.
                        end if
                    end do
                                  ...
                                  react = .TRUE.  !reaction occurrs, so transform A into products
                                    ...
                                    select case (reaction%kinetic%rxn(irxn)%nproducts)
                                        ...
                                        case (2) !we have two products
                                              ...
                                              ! Type_A checks whether C or D is the same as A or B, if so, no need to kill one particle and regenerate another one    - Dong
                                              Type_A = 0
                                              ! Check if any 
                                              If (spC == spA .OR. spD .eq. spA) then
                                                  Type_A = 1
                                                  if (spC == spA) then
                                                      spE = spD
                                                  else
                                              ...
                                              Elseif (spC .eq. spB .OR. spD .eq. spB) then
                                                  Type_A = 2
                                                  if (spC == spB) then
                                                      spE = spD
                                                  else
                                              ...
                                              If (Type_A > 2 .OR. Type_A < 0) stop '>> Check input for A+B->C+D reaction'
                                              
                                              select case (Type_A)
                                                    case (0)                    ! Products and reactants are not the same
                                                        ! Generate a random number at each dimension
                                                        z1 = 2*rand3(seedbimo)-1
                                                        z2 = 2*rand3(seedbimo)-1
                                                        z3 = 2*rand3(seedbimo)-1
                                                    ...
                                                    case (1)                ! One of the reactant is the same as A
                                                        ! Generate a random number at each dimension
                                                        z1 = 2*rand3(seedbimo)-1
                                                        z2 = 2*rand3(seedbimo)-1
                                                        z3 = 2*rand3(seedbimo)-1
                                                        ...
                                                        call generate_plumeparticle_ID_ (ipC) 
                                                        call add_particle_to_plume_ (plume,ipC,xpC,ypC,zpC,mpC,1.d0,izone,spE)
                                                        
                                                        !call delete_plumeparticle_  (plume,izone,spA,partA)     ! particle A in list will not be killed
                                                        Breact(ipB)  = .TRUE.                                   !mark particle B to be killed after loop
                                                        
                                                        exit loopB
                                                        
                                                    case (2)                ! One of the reactant is the same as B
                                                        ! Generate a random number at each dimension
                                                        z1 = 2*rand3(seedbimo)-1
                                                        z2 = 2*rand3(seedbimo)-1
                                                        z3 = 2*rand3(seedbimo)-1
                                                        ...
                                                        call delete_plumeparticle_  (plume,izone,spA,partA)     !kill particle A in list and go to next A particle
                                                        !Breact(ipB)  = .TRUE.                                   ! particle B  will not be killed
             ...
             if (useKDT) call kdtree2_destroy(Btree)
             deallocate(Alocvec,Bloc,nearest)
             if (allocated(distAB)) deallocate(distAB)
