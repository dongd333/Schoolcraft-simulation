...
!********************************************************************************************************************
  ...
  type kinetic_reactions_cl 
      ...
      type(array_cl),     pointer, dimension(:) :: gy => null()         !growth yield coefficient (one for each growth reaction)
 end type
  ...
  function read_kinetic_ (unit,geo) result(this)
      ...
      integer                           :: ngr, igr             ! Add for growth reactions - Dong
	  type(kinetic_reactions_cl)        :: this

      read(unit,*) this%action
      kineticACTION = this%action

	  read(unit,*) nreact, ngr      ! Add number of reactions with growth - DD

      select case (this%action)
	  case (.TRUE.)
	     call alloc_kinetic_ (this,nreact,ngr)
         do irxn=1,nreact
         ...
         ! Read growth yield coefficient for reaction with growth - DD
         if (ngr > 0) then
             Do igr=1,ngr
                 read(unit,*) file,const,ivar,flag
                 this%gy(igr)  = read_array_ (file,ivar,const,flag,geo%nx,geo%ny,geo%nz)
             End Do
         end if
  ...
  subroutine delete_kinetic_ (this) ! destructor
      ...
      if (associated(this%gy))     deallocate (this%gy)
	  !this%action = .FALSE.
  ...
  !************************************
  subroutine alloc_kinetic_ (this,nreact,ngr)
     use global_variables, only: nspecie
     implicit none

	 type(kinetic_reactions_cl), intent(inout) :: this
     integer,                    intent(in)    :: nreact, ngr	 
         ...
         if (.not. associated (this % gy))         allocate (this % gy(ngr))
  end subroutine
