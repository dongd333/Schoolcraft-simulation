...
!...loop over injections
 ...
 source_loop: do isource=1,n	  
          ...
          !...flux-dependent injection
          
          if (trim(adjustl(source%num(isource)%TypeInj)) == 'DEPENDENT') then              
  
                 if (plume%time < source%num(isource)%TimeStartInj .and. plume%time > source%num(isource)%TimeStopInj) cycle source_loop
                 call find_loc_TimeFunction_ (source%num(isource),plume%time)
                 Jm = evaluate_timefunction_upwind_ (source%num(isource)%timefunct,source%num(isource)%loc)     
                 !tstep = plume%time - source%num(isource)%TimeStartInj
                      if (Jm > 0.d0) then                    
                           call inject_QX_FLUX_PARTICLE ( plume, geo, advection, mass_trans, reaction, source, isource, Jm, dt )
                      end if
                 cycle source_loop                
            ...
            do k=nz,1,-1
               do j=ny,1,-1
                   ...
                   do i=1,nx
                      ...
                      np1 = floor(conc(i)*poro*Vol/mp)
                      np2 = mod(conc(i)*poro*Vol/mp,1.0)
...
!      inject particles given one source type
!***********************************************************************************
  subroutine inject_QX_FLUX_PARTICLE ( plume, geo, advection, mass_trans,reaction, source, kinj, mc, ts0 )
  	   use array_class
  	   use mass_trans_class
	   use reaction_class
	   use plume_class
...
!       ==================================================
!       Qx_FLUX-WEIGHTED distribution   % This is added to simulate the flux_dependent boundary: injecting particles based on the flux      - Dong 
!       ==================================================
      ...
      ! get velocities
	    ...
	    iz = source % num (kinj) % par % iz
             ...
             do k=1,size(ix)
                  qx = advection%qx%values(ix(k),iy(k),iz(k))
                  if (ny /= 1) dy = value_array_ (geo%dy,1,iy(k),1)
                  if (nz /= 1) dz = value_array_ (geo%dz,1,1,iz(k))
                  qm(k) = qx*dy*dz
                  poro = value_array_ (advection%poro,ix(k),iy(k),iz(k))
                  
                  if (qm(k) <= 0.d0) return
                  
                  flux_m = mc*ts0*qm(k)*poro/mp
                  ...
                  !npblock = dint( mc*ts0*qm(k)*poro/mp )
                  ...
                  do jp=1,npblock
                    ...
                    if (reaction%action) then
                          ...
                          rpt = particle%prop%rp
                          call generate_plumeparticle_ID_ (id)
                          call add_particle_to_plume_ (plume,id,xx0,yy0,zz0,mp,rpt,zone,specie)
                     else
                          call generate_plumeparticle_ID_ (id)
                          call add_particle_to_plume_ (plume,id,xx0,yy0,zz0,mp,1.d0,zone,specie)
			         end if			       
                 ...
                 !write(*,*) npblock, mc, ts0, qm(k), poro, mp
