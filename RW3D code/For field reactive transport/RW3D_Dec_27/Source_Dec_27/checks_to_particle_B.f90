...
!***************************************************************************************************
  subroutine check_well_arrival (particle,well,reaction,recirculation)
    use gslib, only: root2, open_fname
	use particle_class
	use well_vect_class
	use breakthru_class
	use reaction_class
	...
	real*8                             :: dt
	real*8                             :: xp1,yp1,xp2,yp2,zp1,zp2,zp,xstep,ystep,tp1
    ...
    integer                            :: id, iunit
	   ...
	   if( (d1 <= rwell .and. d2 >= rwell ) .or. (d1 >= rwell .and. d2 <= rwell) )then
			   ...
			   if (.not.associated(well%num(iwell)%zwtop)) then
                  ...
                  if (zp >= well%num(iwell)%zwbot .and.  zp <= well%num(iwell)%zwtop) then        
	                ...
	                if ( well%num(iwell)%out == 1 ) then
                        ...
                        npOut = npOut + 1
                        call open_fname (fname_exit,iunit)
                        write(iunit,101) xp1,yp1,zp,spe,id,tp1,well%num(iwell)%name
                    end if
               ...
               if ( well%num(iwell)%out == 1 ) then
                   call open_fname (fname_exit,iunit)
                   write(iunit,101) xp1,yp1,zp,spe,id,tp1,well%num(iwell)%name
101                format(3(g15.6,x),i4,x,i8,x,g15.6,x,A20)
               end if
...
!****************************************************************************************************
! CHECK PARTICLE ARRIVAL AT A Inner_bdy
!****************************************************************************************************
  subroutine check_Inner_bdy_arrival (particle,Inner_bdy)
     use gslib, only: upper_case, rotate_coord, vectorial_product
	 use particle_class
	 ...
	 use loops_particles, only: ip
	 implicit none
     type(Inner_bdy_vect_cl), intent(inout) :: Inner_bdy
	 type(particle_cl),   intent(inout) :: particle
	 real*8                             :: dt
	 integer                            :: n_Inner_bdy,i_Inner_bdy, spe
     real*8                             :: xp1,yp1,zp1,tp1,xp2,yp2,zp2,xp,yp,zp,xp0,yp0,zp0
	 real*8                             :: xstep,ystep,zstep,dis,time,mp
	 real*8                             :: A,B,C,D,dist_ptTOpl_1,dist_ptTOpl_2,factor
	 real*8                             :: xpart(3),l(3),m(3),n(3),DaI
	 integer                            :: id
      ...
      if (associated(Inner_bdy%num)) then

      n_Inner_bdy = n_Inner_bdy_ (Inner_bdy)  
	  
	  if (n_Inner_bdy <= 0) return
	  ...
	  do i_Inner_bdy=1,n_Inner_bdy
           ...
           if ( upper_case(Inner_bdy%num(i_Inner_bdy)%side) == 'Y' ) then
               if (Inner_bdy%num(i_Inner_bdy)%ls == 0) then
                   if( yp2 <= Inner_bdy%num(i_Inner_bdy)%yinner_bdy ) then
                       particle%control%remove = .TRUE.
                   end if
               else if(Inner_bdy%num(i_Inner_bdy)%ls == 1) then
                   if( yp2 >= Inner_bdy%num(i_Inner_bdy)%yinner_bdy ) then
                       particle%control%remove = .TRUE.
                   end if
               else
                   write(*,*) 'Error on the inner boundary flag'
               end if

           else if ( upper_case(Inner_bdy%num(i_Inner_bdy)%side) == 'X' ) then
               if (Inner_bdy%num(i_Inner_bdy)%ls == 0) then
                   if( xp2 <= Inner_bdy%num(i_Inner_bdy)%xinner_bdy ) then
                       particle%control%remove = .TRUE.
                   end if
               else if(Inner_bdy%num(i_Inner_bdy)%ls == 1) then
                   if( xp2 >= Inner_bdy%num(i_Inner_bdy)%xinner_bdy ) then
