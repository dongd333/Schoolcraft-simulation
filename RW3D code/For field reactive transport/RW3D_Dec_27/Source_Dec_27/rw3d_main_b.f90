...
!    _____________________________________________________________________________________________________________________
     ...
     type(Inner_bdy_vect_cl)    :: Inner_bdy
	 ...
	 real*8                     :: dt                            !time step
	 integer                    :: ibdt(8),ibdtp(8)                       !clock variables to calculate elapsed run time
	 integer                    :: iseed, iseed2
	 integer                    :: ispe,izone,npMobile,i,jp
	 logical                    :: remove, exist0
     
     integer                    :: seed(2), clock, seedbimo, nsim,osp,ii, ioerr        ! For seed generation   - Dong
     integer                    :: nwell, iwell                 ! For checking  - Dong
     character(len=10)    :: chdate, chtime, chzone
     character(len=80)          :: temp1
!    _____________________________________________________________________________________________________________________

     ! Save file to record the number of simulations
     nsim = 1
     inquire(file="Simulations.dat", exist=exist0) 
     if (exist0) then
         ! Read in the "Simulations.dat" file and find the number of last file
         open(10001, file="Simulations.dat")
         read(10001,*)
         ii = 1
         do
             read(10001,'(I12,A80)',iostat=ioerr) osp,temp1
             IF (ioerr < 0) THEN        ! Find the last record to assign new number and exit
                 nsim = osp + 1
                 exit
             END IF
         end do
         close(10001)                   ! close the file after reading and re-open to write
         
         open(10001, file="Simulations.dat", status="old", position="append", action="write")
     else
         open(10001, file="Simulations.dat", status="new", action="write")
         write(10001,'(A12, A20, 5A12)') 'Simulation', 'Time', 'seed1', 'seed2', 'seedbiom', 'iseed', 'iseed2'
     end if
     
!....read parameters and initialize:

	 call print_version   ( version, date )
     call read_parameters ( geo, advection, dispersion, mass_trans, reaction, plume, well, plane, histo, source, recirculation, StressTimes, nsim, Inner_bdy )                                                                 
...
! Generate random seed for the random numbers, so that no need to re-compile every run
     ...
     seedbimo = int(clock/3)                         ! Seed for bimolecular reaction
     iseed = mod(clock/10,int(clock/1000000)*100000)
     iseed2 = 99999-iseed             ! For re-circulation subroutine
     ...
     moves: do nmove=1,nmaxmove     
        ...
        !write(10006,'(I9,F9.4)') nmove, plume%time                      ! - Dong
        ...
        particles: do jp=1,np(ispe,izone)
            ...
            call check_Inner_bdy_arrival            ( particle, Inner_bdy )                                 !check if particle arrived at inner boundaries
        ...
        ! Write the checking to files               - Dong
        
        !write(10004,*)
        !write(10004,'(I9,F9.4,I9)') nmove, plume%time, np
        !write(10007,*) plume%time
     ...
     call date_and_time(chdate,chtime,chzone,ibdtp)
     write(10001,'(I12,1x,i4,A1,i2.2,A1,i2.2,1x,i2,A1,i2.2,A1,i2.2, 6I12)') nsim,ibdtp(1),'/',ibdtp(2),'/',ibdtp(3),ibdtp(5),':',ibdtp(6),':',ibdtp(7), seed(1), seed(2),seedbimo,iseed,iseed2, clock
...
!........calculates elapsed run time:
     ...
     !- Dong
     close(unit=10001)      ! Close the last step summary file 'Simulatons.dat'
