...
!********************************************************************************************************
     subroutine read_parameters  ( geo, advection, dispersion, mass_trans, reaction, plume, well, plane,  &
	                               histo, source, recirculation, StressTimes, nsim, Inner_bdy )
     ...
     type(Inner_bdy_vect_cl),    intent(out) :: Inner_bdy
     ...
     integer                           :: isp,isum,nt,itime,nst,jj,nsim
     real*8                            :: v1,v2
     integer                           :: n_Inner_bdy, i_Inner_bdy 
...
!     Read Dispersion Parameters:
      ...
      ! Initialize the diffusion coefficients for mineral phase (0)     - Dong
      if (nspe_min > 0) then
...
!     Read Inner Boundaries:

	  read(10,*) n_Inner_bdy
      if(n_Inner_bdy>0)  call alloc_Inner_bdy_vect_ (Inner_bdy,n_Inner_bdy)
        do i_Inner_bdy=1,n_Inner_bdy
	      call read_assign_Inner_bdy_ (Inner_bdy%num(i_Inner_bdy),10)
          !write(*,*) Inner_bdy%num(i_Inner_bdy)%xinner_bdy, Inner_bdy%num(i_Inner_bdy)%yinner_bdy
        end do
...
!...........Output Files
      ...
      if (iwbtc == 1) then
          inquire(file="rw3d_BTCs.dat", exist=exist1)             ! Check if the file exist, yes: generate new file with suffix of nsim
           if (exist1) then
               write (nsfn,'(I3.3)') nsim
               files_nam(2)='rw3d_BTCs_'//trim(nsfn)//'.dat'        ! generate new particle paths     - Dong
           end if
