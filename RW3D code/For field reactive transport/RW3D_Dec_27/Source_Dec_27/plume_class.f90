...
!***********************************************************************************************
    ...
    interface print_number_of_particles_ 
         module procedure print_number_of_particles_1,print_number_of_particles_2; end interface
              ...
              zones: do izone = 0, plume%nzone-1
                 ...
                 particles:  do        ! move all particles one time step              
                      if (.not.associated(part)) cycle zones        
	   	              write(iunit,3) part%id, part%xp,part%yp,part%zp,part%mp,part%rp,izone,ispe
	               3  format(i10,5(x,e20.11),2(i4,x))			     
			          part => part%next
			     end do particles
		     end do zones
		    end do species   
...
!************************************************************************
    subroutine print_number_of_particles_1 (plume,dt)
      ...
      integer                                 :: izone,ispe,npl,i
           iunit=6
           write(iunit,'(x,a6,g12.4,x,a4,g12.4,2(x,a5,i8),x,a4,g10.3)') 'TIME =',plume%time,'DT =',dt,'MOVE =',nmove,'NPTOT =',plume%np,'MTOT =',plume%mtot
    end subroutine
