...
C
     ...
     &          TTSMULT,TTSMAX,TMASIN,TMASOT,ERROR,ERROR2,SSMC0,TSLEN
...
C****ECGL ADDTIONS
      ...
      ARGV=' '
C      NUMARGS = IARGC()  
c      IF (NUMARGS.EQ.1) THEN
c        CALL GETARG(2,ARGV)
c        IF (ARGV(1:1).EQ.' ') CALL GETARG(1,ARGV)
c      ENDIF
...
C--WITHIN EACH STRESS PERIOD
     ...
     & X(LCSSMC),SSMC0)
C
C--Recircle subroutine that MSU added in 2005 paper
      DO ICOMP=1,MCOMP
          CALL RECIRC(INSSM,IOUT,KPER,NCOL,NROW,NLAY,NCOMP,
     & IX(LCIB),X(LCCNEW),X(LCCRCH),X(LCCEVT),MXSS,NSS,X(LCSS),
     & X(LCSSMC),SSMC0,ICOMP,MCOMP,TIME2)
      END DO
