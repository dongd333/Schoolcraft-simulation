...
for f = 1:nf
    name = sprintf('rw3d_BTCs_%03d.dat', f+40);
    fileID = fopen(name);
    %read BTCs
    Header = textscan(fileID,'%s',2,'Delimiter','\n');
    %t = zeros(ngrid,nw*nz,nf);
    %c = zeros(ngrid,nw*nz,nf);
    
    for i=1:nw*nz
        Zone   = textscan(fileID,'%s',1,'Delimiter','\n');
        for j=1:ngrid
            data   = textscan(fileID,'%f %f',1,'Delimiter','\n');
            t(j,i,f) = data{1,1};
            c(j,i,f) = data{1,2};
        end
    end
...
%for k=1:nw*nz
%    prt = sprintf('C_Br_%d.csv', k);
%    csvwrite(prt,c(:,k,:));
%end
csvwrite('C_Br_ave_100.csv',c_2);
%% -------------------------------------------------------------------------
h1=figure(1); 
for ii=1:nw
    for jj=1:nz
        ...
        % Plot RW3D outputs
        plot(t(:,kk,1), c_norm(:,kk), 'g-v','markersize',3);   % Plot non-zeros
        hold on;
