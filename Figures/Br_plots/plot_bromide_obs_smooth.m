function [] = plot_bromide_obs_smooth()
close all; clear; clc;
%-------------------------------------------------------
c02=30;
...
xmin=0; xmax=20; ymin=-0.1; ymax=1.3; 

xt=[0 75 150];yt=[0 0.5 1.0];
tlx=0.025; tly=0.025;
pos1=get(0,'screensize');
pos=[pos1(1)+10 pos1(2)+40 pos1(3)-10 pos1(4)-120];

xtext=17;ytext=1.1;
tcutoff = 20;  % Cutoff time
...
bromide = load ('measured_time_conc_bromide_final_r.mat');

for p=1:nw
    for q=1:nz
        Br_meas{p,q}=bromide.(sprintf('c_%d_%d_m', p+8,q*10+25));
        t_br_m{p,q}=bromide.(sprintf('t_%d_%d_m', p+8,q*10+25));
    end
end
% Br_meas reads the measured bromide concentration saved in the mat file
% t_br_m reads the time associated with measured bromide concentrations

%% RT3D Simulation
file2='RT3D005.OBS';  %Tracer Simulation, Bromide

%load the RT3D OBS file:
[b]=read_obs_file(file2,26,0);           %Bromide

%% ------------------------------------------------------------------------
% RW3D results
nf = 10;            % Number of files
ngrid = 50;        % ngrid defined in rt3d input

t = linspace(0.4,tcutoff,ngrid);

% Read in the smoothed breakthrough curve
c(:,:,1) = csvread('C_Br_ave_21.csv');
c(:,:,2) = csvread('C_Br_ave_61.csv');
c(:,:,3) = csvread('C_Br_ave_91.csv');
...
for ii=1:nw
    for jj=1:nz
        ...
        % Plot measurements
        ind4= min(find(t_br_m{ii,jj} >= tcutoff));
        
        plot(t_br_m{ii,jj}(1:ind4),Br_meas{ii,jj}(1:ind4)./c02,'bo','markersize',6,'markerfacecolor','c');
        hold on;
        ...
        hold on;
        errorbar(t(:), c_norm(:,kk),sdv(:,kk)*0.01376*3,'g+','LineWidth',1);
        hold on
        % Mass recovery
        Rec_w = trapz(t, c_norm(:,kk));
        ...
        % Plot RT3D outputs
        ind3=min(find(b(2,:)' >= tcutoff));
        plot(b(2,1:ind3)',b(kk+2,1:ind3)'./c02,'b-');
        hold on;
        
        n_t = ind3;
        br_ts = [];
        t_ts = [];
        br_ts(2:n_t+1) = b(kk+2,1:ind3)'./c02;
        br_ts(1) = 0;               % Add two data to the array (top and end)
        t_ts(2:n_t+1) = b(2,1:ind3)';
        t_ts(1) = t_br_m{ii,jj}(1);
        
        ip_t = interp1(t_ts,br_ts,t_m_v);
        
        % Root mean square error for rt3d
        rms_t=sqrt(sum((ip_t-br_m_v).^2)/length(br_m_v));
        ...
        hold on;
        str = {strcat('Obs: ',num2str(Rec_m)),strcat('RW3D: ',num2str(Rec_w)),strcat('RT3D: ',num2str(Rec_t))};
        text(xtext-7,ytext-0.1,str, 'FontSize', 8);
        hold on;
        text(xtext-16,ytext-0.1,{strcat('RW3D: ', num2str(rms_w)),strcat('RT3D: ',num2str(rms_t))}, 'FontSize', 8);
...
hold on;
%text(0,0,'Time(Days)','Interpreter','LaTex','fontname','times','FontSize',14) % x-axis label
%hold on;
%ylabel('$C/C_0$','Interpreter','LaTex','fontname','times','FontSize',14) % y-axis label
