function [] = plot_initial_Nitrate2()
close all; clear; clc;
%-------------------------------------------------------
c0_Nit=42;
...
for f = 1:nf
    name = sprintf('rw3d_BTCs_%03d.dat', f+6);
    fileID = fopen(name);
    ...
    fclose(fileID);
    t_1 = t(:,1:spe:nw*nz*spe,f);   % Specie 1 only
    c_1 =  c(:,1:spe:nw*nz*spe,f);
end
...
% Take average of the outputs
    ...
    csvwrite('C_Nitrate_ave_10.csv',c_2);
    %csvwrite('C_Nitrate_std_10.csv',sdv);
%% -------------------------------------------------------------------------
h1=figure(1); 
for ii=1:nw
    for jj=1:nz
