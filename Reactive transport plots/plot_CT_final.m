...
for ii=1:nw
    for jj=1:nz
        ...
        % Plot measurements
        t_CT_m{ii,jj} = t_CT_m{ii,jj}(t_CT_m{ii,jj}>0);
        CT_meas{ii,jj} = CT_meas{ii,jj}(t_CT_m{ii,jj}>0);
        ...
        t_CT_m{ii,jj}(ind4) = tcutoff;
        CT_meas{ii,jj}(ind4) = 0;
        ...
        CT_m_v = CT_meas{ii,jj}(1:ind4)./C0_CT;
        t_m_v = t_CT_m{ii,jj}(1:ind4);
        n_w = ngrid;
        ...
        CT_ws(2:n_w+1) = S2(:,kk);
        CT_ws(1) = 1;               % Add two data to the array (top and end)
        %CT_ws(n_w+2) =0;
        t_ws(2:n_w+1) = t(:);
        t_ws(1) = 0;
        ...
        %    t_ws(n_w+2) = 163.905555;
            ...
            ip_w = interp1(t_ws,CT_ws,t_m_v);
            % Root mean square error
            rms_w=sqrt(sum((ip_w-CT_m_v).^2)/length(CT_m_v));
        %else
        %    rms_w = sqrt(sum(CT_m_v.^2)/length(CT_m_v));
        %end
        ...
        CT_ts = [];
        t_ts = [];
        CT_ts(2:n_t+1) = a(kk+2,1:ind3)'./C0_CT*1000;
        CT_ts(1) = 1;               % Add two data to the array (top and end)
        t_ts(2:n_t+1) = a(2,1:ind3)';
        t_ts(1) = t_CT_m{ii,jj}(1);
        
        ip_t = interp1(t_ts,CT_ts,t_m_v);
        %t_ts
        %t_m_v
        %size(CT_ts)
        %size(t_m_v)
        ...
        rms_t=sqrt(sum((ip_t-CT_m_v).^2)/length(CT_m_v));
