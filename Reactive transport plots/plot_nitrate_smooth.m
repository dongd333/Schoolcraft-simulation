...
t = linspace(4.4,tcutoff,ngrid);

% Read in the smoothed breakthrough curve
c(:,:,1) = csvread('C_Nitrate_ave_5.csv');
c(:,:,2) = csvread('C_Nitrate_ave_5_10.csv');
c(:,:,3) = csvread('C_Nitrate_ave_10.csv');
c(:,:,4) = csvread('C_Nitrate_ave_10_15.csv');

% Previous model run
t0 = linspace(0.10555,68,32);
S0(:,:) = csvread('C_Nitrate_S0.csv');
t0 = [t0,linspace(70.938,163.90555,22)]'; % Combine the two time steps
    ...
    csvwrite('C_Nitrate_S_1.csv',c_2);
    csvwrite('C_Nitrate_std_S_1.csv',sdv);
...
for ii=1:nw
    for jj=1:nz
        ...
        %color = hsv(4);
        %for z=1:4
            %plot(t(:), 0.08*c(:,kk,z), '-v','markersize',3,'color',color(z,:));
        %end
        
        %plot(t0(:), S0(:,kk), '-v','markersize',3);   % Plot non-zeros
        %hold on;
