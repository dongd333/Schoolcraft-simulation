function [] = plot_initial_Nitrate_a3()
close all; clear; clc;
%-------------------------------------------------------
c0_Nit=42;
...
for f = 1:nf
    name = sprintf('rw3d_BTCs_%03d.dat', f+29);
    fileID = fopen(name);
    ...
    fclose(fileID);
    t_1(:,:,f) = t(:,1:spe:nw*nz*spe,f);   % Specie 1 only
    c_1(:,:,f) =  c(:,1:spe:nw*nz*spe,f);
    out_name = sprintf('C_Nitrate_%03d.csv', f+29);
    csvwrite(out_name',c_1(:,:,f));
end
...
% Take average of the outputs
    ...
    %csvwrite('C_Nitrate_ave_5_10.csv',c_2);
    %csvwrite('C_Nitrate_std_5_10.csv',sdv);
%% -------------------------------------------------------------------------
h1=figure(1); 
for ii=1:nw
    for jj=1:nz
        ...
        hold on;
        errorbar(t_1(:,kk,1), c_norm(:,kk),sdv(:,kk)./sqrt(10),'g+','LineWidth',1);
        hold on
