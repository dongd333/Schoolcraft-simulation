...
%% RT3D Simulation
%file1='RT3D001.OBS';  % CT Simulation, Bromide

%load the RT3D OBS file:
%[a]=read_obs_file(file2,26,0);           % CT
...
for f = 1:nf
    name = sprintf('rw3d_BTCs_%03d.dat', f+5);
    fileID = fopen(name);
    %fileID = fopen('rw3d_BTCs.dat');
    %read BTCs
    ...
    fclose(fileID);
    t_1(:,:,f) = t(:,4:spe:nw*nz*spe,f);   % Specie 4 only
    c_1(:,:,f) =  c(:,4:spe:nw*nz*spe,f);
    out_name = sprintf('C_CT__%03d.csv', f+0);
    csvwrite(out_name',c_1);
end
...
% Take average of the outputs
    ...
    csvwrite('C_CT_ave_5_10.csv',c_2);
    csvwrite('C_CT_std_5_10.csv',sdv);
%% -------------------------------------------------------------------------
h1=figure(1); 
for ii=1:nw
    for jj=1:nz
        ...
        hold on;
        errorbar(t_1(:,kk,1), c_norm(:,kk),50*sdv(:,kk)./sqrt(10),'g+','LineWidth',1);
        hold on
        
        color = hsv(nf);
        for f = 1:nf
            t_in = t(:,4:spe:nw*nz*spe,f);   % Specie 4 only
            c_in =  c(:,4:spe:nw*nz*spe,f);
            %plot(t_in(:,kk,1), 50*c_in(:,kk), '-v','color',color(f,:),'markersize',1);
            hold on;
        end
